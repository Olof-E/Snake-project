...
{
    // Stall program for a certain amount of cycles
    int i;
    for (i = cyc; i > 0; i--)
        ;
}
...
{
    // Communicate with the OLED display
    while (!(SPI2STAT & 0x08))
        ;
    SPI2BUF = data;
    while (!(SPI2STAT & 0x01))
...
{
    int i, j, k;
    // Return if parameters are wrong or missing
    if (line < 0 || line >= 4)
        return;
    if (!s)
        return;

    // Fill the textbuffer with the given string
    for (i = 0; i < 16; i++)
    {
        if (*s)
        {
    ...
    // Write the textbuffer to the display buffer
    for (i = 0; i < 4; i++)
    {
        DISPLAY_CHANGE_TO_COMMAND_MODE;
        spi_send_recv(0x22);
...
void display_buffer(int x, int size, uint8_t *data)
{
    ...
    // Dsiplay the current display buffer to the screen
    for (i = 0; i < 4; i++)
    {
        DISPLAY_CHANGE_TO_COMMAND_MODE;
...
void display_rect(int x, int y, int size)
{
    ...
    // Find what bit in the byte the rect starts at
    int bytePixelOffset = y - pageIndex * 8;

    // Loop through the rect and fill out the bytes that build up the rect
    for (i = 0; i < size; i++)
    {
        // Declare two bytes in case the rect crosses over two pages
        int pixelByte1 = 0x0;
        int pixelByte2 = 0x0;
        ...
        for (j = 0; j < size; j++)
        {
            // Check if the rect has crossed over two pages and change the byte offset accordingly
            int pageOffset = floor((y + j) / 8) != pageIndex;
            if (pageOffset)
            {
                // Set the bit in byte corresponding to this pixel
                pixelByte2 |= (1 << temp++);
            }
            else
            {
        ...
        // Add the byte to the display buffer
...
// Author: Olof & Tobias
// A function that draws the border around the screen
